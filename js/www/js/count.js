...
function startCountUp(element, index, elem) {
	...
	var id = "countUp_" + countUpID;
	var countNumber = elem.getAttribute('data-count-number');
	var countTotal = elem.getAttribute('data-count-total');
	var countLeft = elem.getAttribute('data-count-left');
	var countPercentage = 100;
	
	if (countTotal === null || countTotal === "" || countTotal === " ") {
		countPercentage = elem.getAttribute('data-count-percentage');
	} else {
		if (countLeft !== null) {
			countPercentage = 100 - ((countNumber / countTotal) * 100);
		} else {
			countPercentage = (countNumber / countTotal) * 100;
		}
	}
	...
	spinner.style.WebkitAnimation = "countUpRotate " + countSpeedPercentage + "s linear forwards";
	spinner.style.animation = "countUpRotate " + countSpeedPercentage + "s linear forwards";
	filler.style.WebkitAnimation = "countUpFiller " + countSpeedPercentage + "s steps(1, end) forwards";
	filler.style.animation = "countUpFiller " + countSpeedPercentage + "s steps(1, end) forwards";
	mask.style.WebkitAnimation = "countUpMask " + countSpeedPercentage + "s steps(1, end) forwards";
